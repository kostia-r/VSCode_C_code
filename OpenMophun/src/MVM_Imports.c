...
#define MVM_TWO_PI               (6.28318530717958647692)
...
static bool MVM_lOpenFileStream(VMGPContext *ctx, VMGPStream *stream, uint32_t mode, uint32_t name_addr)
{
  ...
  {
    if (ctx->file_api.remove)
    {
      (void)ctx->file_api.remove(ctx->file_api.context, ctx->image_path, name);
    }
    memset(stream, 0, sizeof(*stream));
    ctx->regs[VM_REG_R0] = 0U;
    MVM_LOG_D(ctx,
              "file-open",
              "vStreamOpen(file=%s mode=%08X) delete -> %08X\n",
              name,
              mode,
              ctx->regs[VM_REG_R0]);
  ...
  flags = MVM_FILE_OPEN_READ;
  if ((mode & STREAM_WRITE_FLAG) != 0U)
  {
    flags |= MVM_FILE_OPEN_WRITE;
  }
  if ((mode & STREAM_CREATE_FLAG) != 0U)
  {
    flags |= MVM_FILE_OPEN_CREATE;
  }
  if ((mode & STREAM_TRUNC_FLAG) != 0U)
  {
    flags |= MVM_FILE_OPEN_TRUNCATE;
  }

  handle = MVM_FILE_INVALID_HANDLE;
  size = 0U;
  result = ctx->file_api.open
             ? ctx->file_api.open(ctx->file_api.context, ctx->image_path, name, flags, &handle, &size)
             : -1;

  /* Reject host file sizes that cannot be represented by the 32-bit guest ABI. */
  ...
  if (result != 0 || handle == MVM_FILE_INVALID_HANDLE)
  {
    if (handle != MVM_FILE_INVALID_HANDLE && ctx->file_api.close)
    {
      (void)ctx->file_api.close(ctx->file_api.context, handle);
    }
    memset(stream, 0, sizeof(*stream));
    ctx->regs[VM_REG_R0] = 0xFFFFFFFFu;
    if ((flags & MVM_FILE_OPEN_CREATE) == 0U && MVM_lLooksLikeSidecarName(name))
    {
...
static bool MVM_lReadStreamBytes(const VMGPContext *ctx, const VMGPStream *stream, uint32_t pos, void *dst, uint32_t size)
{
  ...
  if (stream->file_handle != MVM_FILE_INVALID_HANDLE)
  {
    size_t read_size;

    read_size = 0u;
    return ctx->file_api.read &&
           ctx->file_api.read(ctx->file_api.context, stream->file_handle, pos, dst, size, &read_size) == 0 &&
           read_size == size;
  }
...
MVM_IMPORT_IMPL(vitoa)
{
  ...
  written = snprintf(digits, sizeof(digits), "%ld", (long)value);
...
MVM_IMPORT_IMPL(vutoa)
{
  ...
  written = snprintf(digits, sizeof(digits), "%lu", (unsigned long)value);
...
MVM_IMPORT_IMPL(vStreamWrite)
{
  ...
  if (stream->file_handle != MVM_FILE_INVALID_HANDLE)
  {
    ...
    if (!ctx->file_api.write || count > UINT32_MAX - stream->pos)
    {
      ctx->regs[VM_REG_R0] = 0xFFFFFFFFU;
      return true;
    }

    end_pos = stream->pos + count;
    if (end_pos > stream->size && ctx->file_api.resize &&
        ctx->file_api.resize(ctx->file_api.context, stream->file_handle, end_pos) != 0)
    {
      ctx->regs[VM_REG_R0] = 0xFFFFFFFFU;
      return true;
    }
    ...
    written_size = 0U;
    if (ctx->file_api.write(ctx->file_api.context,
                            stream->file_handle,
                            stream->pos,
                            MVM_GUEST_CONST_PTR(ctx, buffer, count),
                            count,
                            &written_size) != 0 ||
        written_size > count)
    {
    ...
    writable = (uint32_t)written_size;
    stream->pos += writable;
...
MVM_IMPORT_IMPL(vStreamClose)
{
  ...
  if (stream)
  {
    if (stream->file_handle != MVM_FILE_INVALID_HANDLE && ctx->file_api.close)
    {
      (void)ctx->file_api.close(ctx->file_api.context, stream->file_handle);
    }
...
MVM_IMPORT_IMPL(vGetTickCount)
{
  ...
  {
    ctx->tick_count = ctx->drivers.get_ticks_ms(ctx->drivers.context);
  }
  else
  {
...
MVM_IMPORT_IMPL(vGetRandom)
{
  ...
  if (ctx->drivers.get_random)
  {
    value = ctx->drivers.get_random(ctx->drivers.context);
  }
  else
  {
 ...
 * Ownership: Encoded guest data is borrowed only for the duration of the callback.
 * Blocking: The callback must enqueue or consume the request without retaining guest memory.
 * Status: Partial; encoded beep, MIDI, and AMR data is delegated to the platform.
 */
MVM_IMPORT_IMPL(vPlayResource)
{
  MVM_AudioRequest_t request;
  ...
  /* Stop current playback when requested by the guest. */
  if ((ctx->regs[VM_REG_P2] & 0x00000400U) != 0U)
  {
    if (ctx->drivers.audio_stop)
    {
      ctx->drivers.audio_stop(ctx->drivers.context);
    }
  ...
  /* Map non-stream audio only for the duration of the synchronous callback. */
  if ((ctx->regs[VM_REG_P2] & MVM_AUDIO_FLAG_STREAM) == 0U && ctx->regs[VM_REG_P1] != 0U)
  {
    audio_data = MVM_GuestTryMapConst(ctx, ctx->regs[VM_REG_P0], ctx->regs[VM_REG_P1]);
    if (!audio_data)
    {
      ctx->regs[VM_REG_R0] = 0U;
      return true;
  ...
  format = ctx->regs[VM_REG_P2] & 0xFFU;
  request.data = audio_data;
  request.length = ctx->regs[VM_REG_P1];
  request.flags = ctx->regs[VM_REG_P2];
  request.format = (format == (uint32_t)MVM_AUDIO_FORMAT_BEEP ||
                    format == (uint32_t)MVM_AUDIO_FORMAT_MIDI ||
                    format == (uint32_t)MVM_AUDIO_FORMAT_AMR)
                     ? (MVM_AudioFormat_t)format
                     : MVM_AUDIO_FORMAT_UNKNOWN;

  MVM_EmitEvent(ctx, MVM_EVENT_SOUND_REQUESTED, ctx->regs[VM_REG_P0], ctx->regs[VM_REG_P1]);
  ctx->regs[VM_REG_R0] = (!ctx->drivers.audio_play ||
                          ctx->drivers.audio_play(ctx->drivers.context, &request) == 0)
                           ? 1U
                           : 0U;
 ...
 * Blocking: Non-blocking.
 * Status: Implemented through the platform system-message callback.
 */
...
MVM_IMPORT_IMPL(vPrint)
{
  ...
  MVM_LOG_D(ctx,
            ...
            ctx->bg_color);
#if (MVM_COMPILED_LOG_LEVEL >= 4U)
  if (ctx->log_level >= MVM_LOG_LEVEL_DEBUG)
  {
...
MVM_IMPORT_IMPL(vGetButtonData)
{
  ...
  {
    ctx->button_state = ctx->drivers.input_get_buttons(ctx->drivers.context);
  }
...
MVM_IMPORT_IMPL(vSin)
{
  ...
  radians = ((double)vm_reg_s32(ctx->regs[VM_REG_P0]) / 4096.0) * MVM_TWO_PI;
