...
/* Internal implementation name for the opaque public instance type. */
typedef struct MVM_Instance MpnVM_t;

typedef struct MVM_ImageFile_t
{
...
#if (VMGP_MAX_STREAMS == 0U)
#error "VMGP_MAX_STREAMS must be greater than zero"
#endif

#if (VMGP_MAX_DRAW_COMMANDS != 1U) || (VMGP_MAX_DRAW_PALETTE_SNAPSHOTS != 1U)
#error "The immediate renderer requires exactly one draw command and one palette snapshot"
#endif

#if (VM_HEAP_EXTRA > VM_LOGICAL_HEAP_EXTRA)
#error "Physical guest heap backing cannot exceed the logical heap address range"
#endif

#if ((VM_HIGH_MEMORY_BASE & 3U) != 0U) || ((VM_STACK_GUARD_EXTRA & 3U) != 0U)
#error "Guest high-memory base and stack guard must be 4-byte aligned"
#endif
...
typedef struct VMGPStream
{
  ...
  MVM_FileHandle_t file_handle; /**< Parent-owned named-file handle, or invalid. */
} VMGPStream;
...
struct MVM_Instance
{
  MVM_HostServices_t drivers;      /**< Per-instance parent services copied at initialization. */
  MVM_LogLevel_t log_level;        /**< Highest runtime diagnostic level enabled for this instance. */
  uint32_t log_start_ms;           /**< Platform tick captured when instance logging begins. */
  const char *image_path;           /**< Parent-owned image path retained for named-file resolution. */
  MVM_FileApi_t file_api;           /**< Per-instance copy of the parent filesystem table. */
  MVM_DeviceProfile_t device_profile_storage; /**< Instance-owned copy of the selected profile. */
  const MVM_DeviceProfile_t *device_profile; /**< Selected device profile exposed to platform wrappers. */

  MVM_ImageFile_t image;            /**< Open VM image handle and measured size. */
  ...
  MVM_HeapAllocation_t *heap_allocations; /**< Tracker table carved from the parent-owned runtime pool. */
...
/**
 * @brief Flushes dirty persistent resource overlays through the common file-write callback.
 */
