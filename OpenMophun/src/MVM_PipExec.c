  ...
  switch (op)
  {
    ...
    case OP_STBD:
    {
      ...
      off = ((ext >> 24) == 0x00)
          ? ((entry = MVM_GetVmgpPoolEntry(ctx, vm_imm24_u(ext))) ? MVM_ResolveVmgpPoolValue(ctx, entry) : 0u)
          : (uint32_t)vm_sext24(ext);
      /* PIP memory operands use the low 24 bits; the upper byte may carry guest flags. */
      addr = (ctx->regs[rs] + off) & 0x00FFFFFFu;
    ...
    case OP_SYSCPY:
    {
      dst_addr = ctx->regs[rd] & 0x00FFFFFFu;
      src_addr = ctx->regs[rs] & 0x00FFFFFFu;

      if (!MVM_RuntimeMemRangeOk(ctx, dst_addr, ctx->regs[rt]) ||
          !MVM_RuntimeMemRangeOk(ctx, src_addr, ctx->regs[rt]))
      {
        return false;
      }

      if (!MVM_GuestCopy(ctx, dst_addr, src_addr, ctx->regs[rt]))
      {
        return false;
      }
      ctx->pc += 4;
    ...
    case OP_SYSSET:
    {
      ...
      if (!MVM_GuestSet(ctx, dst_addr, (uint8_t)(ctx->regs[rs] & 0xFFu), ctx->regs[rt]))
      {
        return false;
      }
      ctx->pc += 4;
