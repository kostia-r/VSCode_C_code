 ...
 */
static bool MVM_lReadQueryImage(const MVM_ImageFile_t *image,
 ...
 */
static bool MVM_lParseQueryHeader(const MVM_ImageFile_t *image,
                                  const MVM_InitConfig_t *config,
                                  VMGPHeader *header,
 ...
 */
static uint32_t MVM_lCountQueryResources(const MVM_ImageFile_t *image,
                                         const MVM_InitConfig_t *config,
                                         size_t res_file_offset,
                                         uint32_t res_size);
 ...
 */
static MVM_RetCode_t MVM_lQueryMemory(const MVM_ImageFile_t *image,
                                     const MVM_InitConfig_t *config,
...
const char *MVM_GetVmgpImportName(const VMGPContext *ctx, uint32_t pool_index_1based)
{
  ...
  if (!entry)
  {
    snprintf(bad, sizeof(bad), "<bad:%u>", (unsigned)pool_index_1based);
...
/**********************************************************************************************************************
 *  Name: MVM_QueryMemory
 *  Description: Queries image-dependent runtime-pool requirements from one complete initialization config.
 *********************************************************************************************************************/
MVM_RetCode_t MVM_QueryMemory(const MVM_InitConfig_t *config, MVM_MemReqs_t *requirements)
{
  MVM_ImageFile_t image;
  MVM_RetCode_t result;

  if (!MVM_ValidateConfig(config, false))
  {
    return MVM_INVALID_ARG;
  }

  memset(&image, 0, sizeof(image));
  if (config->file_api->open(config->file_api->context,
                             config->image_path,
                             NULL,
                             MVM_FILE_OPEN_READ,
                             &image.handle,
                             &image.image_size) != 0 ||
      image.handle == MVM_FILE_INVALID_HANDLE)
  {
    return MVM_INIT_FAILED;
  }

  result = MVM_lQueryMemory(&image, config, requirements);
  (void)config->file_api->close(config->file_api->context, image.handle);
  return result;
} /* End of MVM_QueryMemory */
                                     ...
                                     MVM_MemReqs_t *requirements)
{
  ...
  if (!image || !config || !config->file_api || !config->file_api->read ||
  ...
  requirements->logical_guest_address_space_bytes = logicalMemoryBytes;
  ...
  requirements->allocator_metadata_bytes =
      (size_t)VMGP_MAX_HEAP_TRACKED_ALLOCATIONS * sizeof(MVM_HeapAllocation_t);
  runtimePoolBytes = MVM_lAlignPoolSize(0u);
  runtimePoolBytes += requirements->allocator_metadata_bytes;
  ...
  if (config->services.display_flush)
  {
    framebufferPixelCount = (size_t)config->profile.screen_width *
                            (size_t)config->profile.screen_height;
    ...
    requirements->framebuffer_bytes = framebufferPixelCount * sizeof(uint16_t);
    runtimePoolBytes += requirements->framebuffer_bytes;
  }
                                ...
                                size_t size)
{
  ...
  readSize = 0U;
  return config->file_api->read(config->file_api->context,
                                image->handle,
                                offset,
                                dst,
                                size,
