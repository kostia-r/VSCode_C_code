 ...
 *           Module:  MVM_Core
 *           Target:  Portable C
 *      Description:  Shared VM logger interface and level-gated log macros.
 *********************************************************************************************************************/
...
#define MVM_LOG_RAW(ctx, level, event, fmt, ...) \
  ...
  { \
    ...
    { \
      MVM_LogMessage((ctx), (level), (fmt), ##__VA_ARGS__); \
    } \
  } while (0)
...
#else
#define MVM_LOG_E(ctx, event, fmt, ...) ((void)0)
#endif

#if (MVM_COMPILED_LOG_LEVEL >= 2U)
