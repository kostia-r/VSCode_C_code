...
The source of truth for scope and current work is `MVM/ROADMAP.md`. Phase 11
...
- `MVM/` - portable VM component.
  - `inc/` - public API (`MVM.h`, `MVM_Types.h`, device/profile interfaces).
  - `core/` - VM state, guest memory, stack, and bounded execution.
  - `loader/` - VMGP image, pool, string, and resource metadata loading.
  - `pip/` - PIP/PIP2 instruction decode and execution.
  - `runtime/` - imports, streams, rendering, decompression, timing, and dispatch.
  - `debug/` - trace and VMGP inspection helpers.
  - `Config/` - bundled integration configuration and platform bindings.
  - `vm.mk` - reusable source/include list for embedding builds.
- `Src/` - Windows/SDL runner, input scripts, recording, audio, and host loop.
...
- `Examples/` - platform integration examples/templates.
...
- `MpnVM_t` is opaque. Hosts allocate aligned storage through
  `MVM_GetStorageSize()`, `MVM_GetStorageAlign()`, and `MVM_GetVmFromStorage()`.
...
- Follow `MVM/STYLE_GUIDE.md` for component code. In particular: two-space
...
- Keep `Src/main.c` thin. Put host-loop logic in `VmRunner`, SDL behavior in
  `SdlBackend`, and VM semantics in the library.
...
- Update `MVM/ROADMAP.md` when a milestone or defect status materially changes;
